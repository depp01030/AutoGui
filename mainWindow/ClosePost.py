...
def get_job_id(gen_object):
    ...
    if 'JOB' in os.environ.keys() and os.environ['JOB']:
        ...
        attr_dict = gen_object.DO_INFO('-t job -e %s -d ATTR' % (os.environ['JOB']))
        ...
        # if not comment:
            ...
            #                 job=%s,name1=,type=Job,value=%s" % (os.environ['JOB'], comment_text))
        if comment != '':
...
def main():
    ...
    try:
        if sys.version_info[0] + 0.1 * sys.version_info[1] < 2.6:
            return 
        if 'JOB' in os.environ.keys() and os.environ['JOB']:
            gen_object = genClasses.Top()#(os.environ['JOB'])   
        else:
            return 
        ...
        if os.path.isfile(file_path):
            ...
            for job_log in job_logs:
                try:
                    job_log_dict = eval(job_log)# .encode('latin1').decode('utf-8')
                except:
                    continue
